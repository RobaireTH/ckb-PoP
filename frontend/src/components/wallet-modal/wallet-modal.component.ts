  ...
  template: `
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
      ...
      <div class="relative transform overflow-hidden rounded-3xl bg-zinc-900 border border-white/10 px-4 pb-4 pt-5 text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-sm sm:p-6 animate-scale-in max-h-[80vh] overflow-y-auto">
        <div>
          <div class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-lime-400/10">
        ...
        @if (!walletService.isConnecting()) {
                ...
                <div class="grid gap-2">
                  @for (signerInfo of wallet.signers; track signerInfo.name) {
                    <button
                      ...
                      class="group flex w-full items-center justify-between rounded-xl border border-white/5 bg-white/5 px-4 py-3 text-sm font-semibold text-zinc-200 shadow-sm hover:bg-white/10 hover:border-lime-400/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
          ...
          <div class="mt-8 flex flex-col items-center gap-4">
            <div class="h-16 w-16 rounded-full border-2 border-zinc-800 border-t-lime-400 animate-spin"></div>
